function [Gain, errMsg] = my_bst_openmeeg(OPTIONS)
...
OpenmeegDir = bst_fullfile(cd, 'openmeeg', osType);
...
nVert    = [];
nFaces   = [];
for i = 1:length(OPTIONS.BemSurf)
    % Output MESH file
    trifiles{i} = bst_fullfile(TmpDir, sprintf('openmeeg_%d.tri', i));
    % Write MESH in tmp folder
    [nVert(i),nFaces(i)] = my_out_tess_tri(OPTIONS.BemSurf{i}, trifiles{i}, 1);
end
% Write geometry file
...
if isEeg
    ...
    Gain = NaN * zeros(length(OPTIONS.Channel.Loc), 3 * nv);
    if  OPTIONS.isAdjoint
    ...
    Gain = om_load_full(eeggain_file);
end
% === MEG ===
if isMeg
    % Compute DS2MM
    Gain = NaN * zeros(length(OPTIONS.iMeg), 3 * nv);
    ...
    Gain = om_load_full(meggain_file);
end
% === ECOG ===
% if isEcog
